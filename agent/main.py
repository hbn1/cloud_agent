...
async def run_interactive_mode(
    ...
    """运行与多智能体图的交互式聊天循环。"""
    print("\n" + "=" * 60)
    print("[Agent] Cloud Platform Multi-Agent System Ready!")
    ...
    print(f"\n  [MEM] Short-term (Redis) : {'[OK] connected' if st_ok else '[ERROR] not available'}")
    print(f"  [MEM] Long-term  (Milvus): {'[OK] connected' if lt_ok else '[ERROR] not available'}")
    print()
    ...
    try:
        while True:
            ...
            # 1. 在执行前获取内存上下文
            print("[Memory] Retrieving memory context...")
...
async def main() -> None:
    ...
    try:
        if args.query:
            ...
            print(f"\n👤 User: {args.query}")
            result = await graph.ainvoke(state)
            print(f"\n[Agent] AI: {result['messages'][-1].content}\n")
        else:
