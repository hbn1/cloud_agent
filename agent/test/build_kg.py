...
def extract_knowledge_graph(file_path: str) -> dict:
    ...
    # ChatOpenAI 使用 Pydantic v2 model_fields，Pylance 无法识别命名参数
    # 通过环境变量传入 api_key 和 base_url，工厂函数规避 constructor 类型检查
    os.environ["OPENAI_API_KEY"] = api_key
    os.environ["OPENAI_BASE_URL"] = base_url
    def _llm(**kw: Any) -> ChatOpenAI:
        return ChatOpenAI(**kw)
    llm = _llm(model=model_name, temperature=0)
    ...
    print(f"[Split] 文档已切分为 {len(chunks)} 个片段，开始逐块提取...")

    all_nodes = {}  # 使用 dict 去重，key 为 node_id
    all_edges = set() # 使用 set 去重，存储 tuple(source, type, target)
    
    for i, chunk in enumerate(chunks):
        print(f"[Wait] 正在处理第 {i+1}/{len(chunks)} 个片段...")
        try:
            kg_result = cast(KnowledgeGraph, chain.invoke({"text": chunk}))
            ...
            print(f"[Warn] 第 {i+1} 个片段提取失败，跳过: {e}")
            continue

    # 重新组装最终的 KnowledgeGraph
    final_kg = {
        "nodes": [node.model_dump() for node in all_nodes.values()],
    ...
    print(f"[OK] 所有片段提取并合并完成！共提取 {len(final_kg['nodes'])} 个节点，{len(final_kg['edges'])} 条关系。JSON 已保存至: {output_json}")
    return final_kg
...
def import_to_neo4j(kg_data: dict):
    ...
    print(f"[Connect] 正在连接 Neo4j 数据库 ({uri})...")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(md_file_path):
        print(f"[ERROR] 找不到文件: {md_file_path}")
        sys.exit(1)
