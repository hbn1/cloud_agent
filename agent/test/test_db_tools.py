...
def test_db_tools():
    ...
    if parsed_res1.get("status") == "success":
        print(f"[OK] 成功查询到 user_1001 的订单数: {len(parsed_res1['data'])}")
        print(json.dumps(parsed_res1['data'][:1], ensure_ascii=False, indent=2))
    else:
        print(f"[ERROR] 查询失败: {parsed_res1.get('message')}")
