...
async def stream_chat(query: str, user_id: str, session_id: str):
    ...
    try:
        ...
        if cache_hit:
            ...
            clean_query, security_flags, blocked = security_gate.sanitize(query)
            if blocked:
                response_text = "您的消息包含不安全的输入，已被拦截。如有疑问请联系客服。"
            else:
                if security_flags:
                    print(f"[Security] PII redacted: {security_flags}")
                # Send status event to frontend
                status_msg = json.dumps({"status": "thinking", "message": "正在分析您的问题..."})
        ...
        status_msg2 = json.dumps({"status": "responding", "message": "正在生成回答..."})
        yield "data: " + status_msg2 + "\n\n"
        ...
        for i in range(0, len(response_text), chunk_size):
            if cancel_event.is_set():
                print(f"[Stop] Session {session_id} cancelled during output")
                yield f"data: {json.dumps({'done': True, 'cancelled': True})}\n\n"
                return
